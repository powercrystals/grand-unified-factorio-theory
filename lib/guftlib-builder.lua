...
function builder.build_ore_crushing_recipe(ore, subgroup)
	if not data.raw.item[ore] or not data.raw["recipe-category"]["ore-refining-t1"] then
		return
	end
...
function builder.build_gem_crushing_recipe(gem, subgroup)
	if not data.raw.item[gem] or not data.raw["recipe-category"]["ore-refining-t1"] then
		return
	end
